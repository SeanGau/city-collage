  ...
  if ($(this).data("src") == "openmap") {
    ...
    async function initMap() {
      ...
      map = new Map(document.getElementById("map"), {
        zoom: 20,
        center: { lat: 25.040510, lng: 121.513473 },
        mapTypeId: 'satellite',
...
$("#mapModal #capture").on("click", function () {
  const streetView = map.streetView;
  let info = {};
  ...
  let staticUrl = "";
  if (info.type === 'topdown') {
    staticUrl = `https://maps.googleapis.com/maps/api/staticmap?center=${info.lat},${info.lng}&zoom=${info.zoom}&size=640x640&scale=2&maptype=satellite&key=${gmpApiKey}`;
  } else {
